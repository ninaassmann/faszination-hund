import type { FeaturedList as FeaturedListProps, Dog, Dogbreed } from '@/payload-types'
        ...
        <div className="mt-8 grid gap-6 grid-cols-3">
          {type === 'dogs' &&
            featuredDogs?.filter(isDog).map((dog) => {
              ...
              return (
                <article key={dog.id} className="card bg-base-100 shadow-sm overflow-clip">
                  {imageUrl ? (
          ...
          {type === 'dogbreeds' &&
            featuredBreeds?.filter(isDogbreed).map((breed) => {
              ...
              return (
                <article key={breed.id} className="card bg-base-100 shadow-sm overflow-clip">
                  {imageUrl ? (
                    <div className="bg-gray-200 w-full h-64 flex items-center justify-center aspect-[3/4] relative">
                      <Image src={imageUrl} alt={imageAlt} fill className="object-cover" />
                    </div>
